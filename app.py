...
def update_matches():
    ...
    try:
        for match in matches_data:
            ...
            if not cur.fetchone():
                cur.execute(
                    """INSERT INTO matches (api_match_id, home_team, away_team, kickoff_time, deadline, status, home_score, away_score, league)
                       VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)""",
                    (str(api_id), home_team, away_team, kickoff_utc.strftime("%Y-%m-%dT%H:%M:%S"), deadline_utc.strftime("%Y-%m-%dT%H:%M:%S"), status, home_score, away_score, league))
            else:
                cur.execute(
                    """UPDATE matches SET status=%s, home_score=%s, away_score=%s, kickoff_time=%s, deadline=%s, league=%s WHERE api_match_id=%s""",
...
def index():
            ...
            try:
                ...
                if home_goals >= 0 and away_goals >= 0:
                    ...
                    if m and m[4] in ('SCHEDULED', 'TIMED') and is_before_deadline(m):
                        cur.execute(
                            """INSERT INTO predictions (user_id, match_id, home_goals, away_goals) VALUES (%s, %s, %s, %s)
                        ...
                        flash(f"✅ Ставка принята", "success")
                    else:
    ...
    try:
        if league_filter == 'all':
            cur.execute(
                """SELECT id, home_team, away_team, kickoff_time, deadline, status, league, home_score, away_score
                   FROM matches WHERE status IN ('SCHEDULED', 'TIMED', 'FINISHED') AND kickoff_time >= %s ORDER BY kickoff_time""",
                (start_date_str,))
        else:
            cur.execute(
                """SELECT id, home_team, away_team, kickoff_time, deadline, status, league, home_score, away_score
                   FROM matches WHERE status IN ('SCHEDULED', 'TIMED', 'FINISHED') AND league=%s AND kickoff_time >= %s ORDER BY kickoff_time""",
                (league_filter, start_date_str))
        raw_matches = []
        for m in cur.fetchall():
            raw_matches.append({
                'id': m[0], 'home_team': m[1], 'away_team': m[2], 'kickoff_time': m[3],
                'deadline': m[4], 'status': m[5], 'league': m[6], 'home_score': m[7], 'away_score': m[8]
            })
        ...
        for match in raw_matches:
            ...
            if match['finished']:
                ...
                pts = cur.fetchone()
                match['my_points'] = pts[0] if pts else 0
    ...
    try:
        if request.method == 'POST':
            ...
            if action == 'update_matches':
                update_matches_safe(); flash("Обновлено", "success")
                ...
                try:
                    ...
                    cur.execute("""INSERT INTO matches (home_team, away_team, kickoff_time, deadline, status, league)
                        ...
                        (home, away, utc.strftime("%Y-%m-%dT%H:%M:%S"), deadline_utc.strftime("%Y-%m-%dT%H:%M:%S"), league))
...
def admin_translate():
    ...
    try:
        ...
        matches = cur.fetchall(); updated = 0
        for m in matches:
            new_home = translate_name(m[1]); new_away = translate_name(m[2])
            if new_home != m[1] or new_away != m[2]:
                cur.execute("UPDATE matches SET home_team=%s, away_team=%s WHERE id=%s", (new_home, new_away, m[0]))
                updated += 1
...
def login():
    if request.method == 'POST':
        ...
        if user:
            session['user_id'] = user[0]; session.permanent = True
...
def register():
    if request.method == 'POST':
        ...
        try:
            ...
            flash("Регистрация успешна", "success"); return redirect(url_for('login'))
