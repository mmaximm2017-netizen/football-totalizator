...
from scripts.auto_result_worker import decide, window_state
...
def test_livesport_requires_ok_and_ignores_penalty_score():
    ...
    finished = find_livesport_result(html, home="Динамо", away="Ахмат", match_date="2026-09-02")
    assert finished.status == STATUS_FINISHED
    assert (finished.home_score, finished.away_score) == (1, 1)
    waiting = find_livesport_result(html, home="Динамо", away="Ахмат", match_date="2026-09-03")
...
def test_sports_rpl_requires_match_finished_text():
    ...
    <article class="calendar-card" data-match-status="FINISHED">
      ...
      <span class="calendar-score__score">2</span><span class="calendar-score__score">2</span>
    </article>
    """
    result = find_sports_rpl_result(finished_html, home="Акрон", away="ЦСКА", match_date="2026-08-28")
    assert result.status == STATUS_FINISHED
    assert (result.home_score, result.away_score) == (2, 2)

    future_html = finished_html.replace("FINISHED", "NOT_STARTED").replace("Матч окончен", "16:30").replace(">2<", ">-<")
    result = find_sports_rpl_result(future_html, home="Акрон", away="ЦСКА", match_date="2026-08-28")
    assert result.status == STATUS_NOT_FINISHED


def test_sportbox_requires_timeline_end_not_merely_score():
    final = json.dumps({"live": 0, "score": "3 : 0", "football_timeline_html": '<div class="b-timeline b-timeline-end"></div>'})
    result = parse_sportbox_game_json(final, match_date="2026-06-09")
    assert result.status == STATUS_FINISHED
    assert (result.home_score, result.away_score) == (3, 0)

    not_final = json.dumps({"live": 0, "score": "3 : 0", "football_timeline_html": '<div class="b-timeline"></div>'})
...
def test_window_starts_at_120_and_hard_ends_after_180_with_one_cron_grace():
    ...
    assert window_state(kickoff, datetime(2026, 9, 3, 18, 29, 59, tzinfo=timezone.utc)) == "too_early"
    assert window_state(kickoff, datetime(2026, 9, 3, 18, 30, tzinfo=timezone.utc)) == "active"
    assert window_state(kickoff, datetime(2026, 9, 3, 19, 30, tzinfo=timezone.utc)) == "active"
    assert window_state(kickoff, datetime(2026, 9, 3, 19, 31, tzinfo=timezone.utc)) == "expired_grace"
    assert window_state(kickoff, datetime(2026, 9, 3, 19, 36, tzinfo=timezone.utc)) == "expired"
